...
{
  testthat::test_that("Format has the correct names", {
    correct_names <- c("path_prefix", "nlon", "nlat", "dx",
                       "dy", "xmin", "ymin", "variables")
    all(names(ed_metheader_format) %in% correct_names)
  })
  testthat::test_that("ED met header files exist and are not empty",
                      {
                        ...
                        testthat::expect_true(all(file.size(met_files) >
                                                    0))
                      })
  testthat::test_that("Met header metadata fields are valid",
                      {
                        ...
                        testthat::expect_is(ed_metheader_format$variables,
                                            "data.frame")
                      })
  if (check_files) {
    met_files <- PEcAn.utils::match_file(ed_metheader_format$path_prefix,
