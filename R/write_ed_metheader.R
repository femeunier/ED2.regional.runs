...
write_ed_metheader <- function(ed_metheader, filename,
  ...
  for (i in seq_len(nformats)) {
    ...
    block_lines[3] <- 1
    block_lines[4] <- nrow(metformat$variables)
    block_lines[5] <- paste(metformat$variables$variable, collapse = " ")
    block_lines[6] <- paste(metformat$variables$update_frequency, collapse = " ")
    block_lines[7] <- paste(metformat$variables$flag, collapse = " ")
